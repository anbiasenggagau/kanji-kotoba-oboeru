...
function addCorrectAnswer() {
    nextKanji(addCorrectAnswerCallback)
}
...
function addWrongAnswer() {
    nextKanji(addWrongAnswerCallback)
}
        ...
        <div class="flex flex-col justify-center items-center min-h-[100dvh] space-y-4">
            ...
            <div class="card flex justify-center">
                <Transition name="fade" mode="out-in">
                    <template v-if="revealButton">
                        <Button class="text-sm md:text-base" key="reveal" label="Lihat Jawaban" @click="revealAnswer" />
                    </template>
                    <template v-else>
                        <div key="mark" class="flex space-x-2">
                            <DangerButton class="text-sm md:text-base" @click="addWrongAnswer"
                                label="Tandai Sebagai Salah" variant="outlined" />
                            <Button class="text-sm md:text-base" @click="addCorrectAnswer" label="Tandai Sebagai Benar"
                                variant="outlined" />
                        </div>
                    </template>
                </Transition>
            ...
            <div class="card flex justify-center">
                <Transition name="fade" mode="out-in">
                    ...
                    <template v-else>
                        <div class="hidden lg:flex space-x-40">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 640 640">
                                <path
